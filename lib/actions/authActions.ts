...
export async function registerUser(user: Omit<User,"id" | "emailVerified" | "image" | "firstName">){
...
export async function forgotPassword(email:string){
    ...
    const body = compileResetPassTemplate(user.email, resetPassUrl);
    const sendResult = await sendMail({to:user.email, subject:"Reset Password", body:body});
    return sendResult;
}
